...
# Create the different dice probabilities of each outcome
tetra_pf = p_dice(4)
cube_pf = p_dice(6)
octa_pf = p_dice(8)
dodeca_pf = p_dice(12)
...
# Method to create a random number for a given dice
def rand_dice(n):
    return np.random.randint(1, n + 1)


# Monte-Carlo method for number of trials
def monte(trial):
    win = 0
...
def monte_trials(n, max_trials=100000):
    ...
    while n < max_trials:
        ...
        n_trials.append(n)
        n += 1000
    # Returns the number of trials and the relative error of each trial
    return n_trials, end_result


task_5 = monte_trials(1000)

# Plots for the different tasks
